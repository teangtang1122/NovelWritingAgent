...
Moshu 2.0 将章节、角色、大纲、关系网和世界观等创作资料迁移到用户选择的本地小说数据目录中，使用 Markdown / JSON 文件作为可读写的主要内容源。SQLite 仍保留为索引、运行日志、任务状态、配置和兼容层。

首次启动新版 exe 时，用户需要选择一个空目录作为小说数据目录。如果本机已经存在旧版数据库数据，Moshu 会在启动时把旧作品导出到该目录，并将数据库记录切换到文件目录模式。之后 Claude Code、Codex 和其他外部 Agent 可以通过 MCP 工具读取、搜索、写入这些项目文件，再调用同步工具让前端和数据库索引保持一致。
...
长内容不要直接塞进一次工具参数里，可以先写入项目目录下的文件，再用 `content_ref` 或同步工具交给墨枢处理。
