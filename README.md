...
墨枢现在可以自行管理本地大模型，不再要求用户必须购买 API 或安装开发环境。

1. 打开桌面控制面板中的“本地 AI”。
2. 墨枢会检测 CPU、内存、显卡和显存，并推荐 Qwen3 4B、8B 或 14B 的 GGUF 模型。
3. 点击安装后，程序会自动下载 `llama.cpp` 运行时和模型；支持 Hugging Face、ModelScope 双源切换、断点续传和文件校验。
4. 下载完成后可将本地模型设为默认。项目助手、作品建档、新书规划、章节写作、角色与世界观更新、质量评估都可以离线运行。

模型权重不会打包进 `Moshu.exe`。默认存放于 `%LOCALAPPDATA%\Moshu\models`，可在模型中心修改目录。运行时会根据显存自动调整 GPU 层数和上下文长度，加载失败时依次降级到较低显存模式和 CPU 模式。

“LoRA 训练 Beta”支持在 NVIDIA 显卡上从用户拥有权利的章节、修改前后版本和角色对白生成训练集，并训练私人或作品专属写作适配器。训练环境首次使用时单独下载，训练文本和产物不会上传。
