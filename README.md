...
推荐方式：在 Moshu 的系统设置里新增或切换到 `Claude Code CLI` / `Codex CLI` 本机提供商并保存。Moshu 会自动把自己的 MCP Server 以 `--permission-pack auto` 添加到对应客户端，只替换名为 `moshu` 的 MCP 配置，并保留用户已有的其他 MCP Server。默认启用可信本地模式：外部 Agent 可以读取项目文件镜像并调用 Moshu MCP 项目工具，不会暴露 API Key、模型密钥或内部模型额度工具。
