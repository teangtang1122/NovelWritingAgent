...
def _strip_json_fences(text_result: str) -> str:
...
def _extract_json(text_result: str) -> dict:
    """Best-effort JSON extraction from model output."""
    cleaned = _normalize_json_punctuation(_strip_json_fences(text_result))
    if not cleaned:
    ...
    try:
        parsed, _ = decoder.raw_decode(_remove_trailing_commas(candidate))
        return parsed
    except json.JSONDecodeError as first_error:
        end = candidate.rfind("}")
        if end > 0:
            try:
                return json.loads(_remove_trailing_commas(candidate[:end + 1]))
            except json.JSONDecodeError:
                pass
...
def _parse_model_json(text_result: str) -> tuple[Optional[dict], Optional[str]]:
    try:
        return _extract_json(text_result), None
...
def _map_instructions(options: dict) -> str:
    parts = [
        "【分块事实卡片要求】",
...
def _reduce_template(options: dict) -> str:
    optional_fields = []
...
def _reduce_instructions(options: dict) -> str:
    instructions = [
        "【整合规则】",
...
async def _reduce_section(
    ...
    for attempt in range(REDUCE_PARSE_RETRIES):
        ...
        parsed, error = _parse_model_json(text_result)
    ...
    return [
        {"role": "system", "content": MAP_SYSTEM_PROMPT},
        {
            ...
            "content": (
                ...
                "分析以下小说文本片段（第{}段）：\n\n{}{}"
            ).format(_map_instructions(options), MAP_OUTPUT_RULES, MAP_JSON_TEMPLATE, index + 1, chunk[:4000], retry_tip),
        },
    ]
...
async def _repair_json_output(raw_text: str, model: Optional[str]) -> tuple[Optional[dict], Optional[str], str]:
    ...
    parsed, error = _parse_model_json(repaired_text)
    if parsed is not None:
...
async def _stream_map_chunk(
    ...
    for attempt in range(MAP_PARSE_RETRIES):
        ...
        if full_text.strip():
            last_raw = full_text
        parsed, error = _parse_model_json(full_text)
        if parsed is not None:
            if attempt > 0:
                parsed["_retry_count"] = attempt
